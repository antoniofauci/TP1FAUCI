...
public class ConsoleApp implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        log.info("Montant des articles pour une commande");
        Integer numeroCommande = 1; // Remplace par un numéro valide
        BigDecimal montant = commandeDAO.montantArticles(numeroCommande);
        if (montant != null) {
            log.info("Montant total des articles pour la commande {} : {}", numeroCommande, montant);
        } else {
            log.info("Aucune commande trouvée pour le numéro {}", numeroCommande);
        }
